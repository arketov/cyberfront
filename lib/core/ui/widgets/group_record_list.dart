...
class _GroupRecordListState extends State<GroupRecordList> {
  ...
  Future<List<RecordGroupDto>> _getFuture(int limit, int? trackId, int? carId) {
    final key = _RecordsKey(limit, trackId, carId);
    return _futureByKey.putIfAbsent(
      key,
      () => _api.getTopGroups(limit: limit, trackId: trackId, carId: carId),
    );
  }
  ...
  void _retry(int limit, int? trackId, int? carId) {
    ...
    setState(() {
      ...
      _futureByKey[key] = _api.getTopGroups(limit: limit, trackId: trackId, carId: carId);
    });
  }
