// lib/features/tracks/details/cards/track_run.dart
...
import 'package:cyberdriver/features/tracks/data/track_run_api.dart';
...
class TrackRunCard extends StatefulWidget {
  const TrackRunCard({super.key, required this.trackId});

  final int trackId;

  @override
  State<TrackRunCard> createState() => _TrackRunCardState();
}

class _TrackRunCardState extends State<TrackRunCard> {
  late final TrackRunApi _api;
  AuthService? _auth;
  Future<UserTrackRunDto>? _future;
  bool _authReady = false;

  @override
  void initState() {
    super.initState();
    _api = TrackRunApi(createApiClient(AppConfig.dev));
    _initAuth();
  }
  ...
  Future<void> _initAuth() async {
    ...
    setState(() {
      ...
      _authReady = true;
      if (userId > 0) {
        _future = _api.getTrackRunWithAuth(auth, userId, widget.trackId);
      }
    });
  }
...
class _RunCardShell extends CardBase {
  ...
  final Future<UserTrackRunDto>? future;
  ...
  Widget buildContent(BuildContext context) {
    ...
    return FutureBuilder<UserTrackRunDto>(
      future: future,
